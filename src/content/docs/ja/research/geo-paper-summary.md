...
|------|------------|-------------------|
| **統計データの追加** | **+115.1%** | 引用シグナル |
| **ソースの引用** | **+77.0%** | 引用シグナル |
| **引用文の追加** | **+72.2%** | 権威性シグナル |
| 権威的なトーン | +21.5% | ナレッジクラリティ |
| 流暢さの最適化 | +15.2% | ナレッジクラリティ |
| 技術用語 | +5.8% | ナレッジクラリティ |
| 平易な表現 | +2.4% | ナレッジクラリティ |
| ユニークな語彙 | -3.1% | -- |
| キーワード詰め込み | -10.2% | -- |
...
### 1. 引用シグナルが最もレバレッジの高いコンポーネント

GEOのデータは、引用シグナル（統計データ、ソース、引用文）が最大の可視性改善をもたらすことを示している。LLMOフレームワークが引用シグナルをコンポーネント5（他のすべてのコンポーネントの効果を乗算するキャップストーン）として位置づけているのはこのためだ。

### 2. コンテンツの明快さは重要だが、エビデンスほどではない

ナレッジクラリティに関連する戦略（権威的なトーン、流暢さ、平易な表現）はすべてプラスだが控えめな改善（2-22%）を示した。良い文章は必要だが十分ではない。真のレバレッジは検証可能な事実の追加にある。
...
3. **検索レイヤー**: GEOはコンテンツがすでに取得されていることを前提としている。LLMOはコンテンツが最初に発見可能であることを保証する検索シグナルコンポーネントを追加する。

## 参考リンク

- [arXivの論文全文](https://arxiv.org/abs/2311.09735)
- [LLMOフレームワーク概要](/ja/framework/overview/)
- [引用シグナル](/ja/framework/citation-signals/) — GEOで最も効果的な戦略の実装方法
