...
> **構造化フォーマットとの境界**: *構造化フォーマット* は「各面が個別に well-formed か?」を問う — 有効な JSON-LD、セマンティック HTML、スコープされたスキーマ。*整合性シグナル* は「面同士が一致しているか?」を問う — HTML と Markdown で同じ住所、`llms.txt` とトップページプロフィールで同じ数値、矛盾するフィールドを持つ2つの `Organization` エンティティが存在しないこと。あるページは構造化フォーマットをパスしながら整合性で失敗しうる: 各ブロックは妥当だが、まとめると2つの物語を語っている。
...
LLMO の意味において、バージョン番号は事実だ — AI が引用できるサイトに関する主張。`package.json` が `1.2.0` と言い、`src/data/version.ts` が `1.1.0` と言い、changelog ページが英語版で `v1.2.0` 日本語版で `v1.1.0` と言い、最新の git タグが `v1.1.0` のとき、サイトは同じ事実について5つの面にわたって自分自身と矛盾している。

これは理論ではない。あなたが読んでいるフレームワークが、まさに v1.2.0 でそのドリフトを出した。[自社監査ケーススタディ](/ja/case-studies/llmo-framework-self-audit/) に何が起きたかが記録されている。

これを防ぐパターン:

1. **可能な限り多くのバージョン面を1つのソースから生成する**。`package.json` + 型付きデータモジュール + changelog Markdown を一緒に更新する bump スクリプトは、整合性を価値として掲げるフレームワークにとって必須インフラだ
2. **バージョンをメタデータの中ではなく実行時に可視化する**。型付きデータモジュールから読んで `v{VERSION}` を表示するフッターは、ビルド時のドリフトを即座にユーザー側のフィードバックに変える。`npm run build` を走らせた保守者は、全ページの下部で食い違いに気づく
3. **リリースをクロスチェックでゲートする**。`package.json` のバージョンを読み、`CHANGELOG.md` / `src/data/version.ts` / changelog ページに同じ値があるかを grep し、不一致なら非零で exit する CI ステップを置く
4. **タグを打つ前に read-only AI 第2段レビューを走らせる**。コストは API トークンで数セント、便益はユーザーが先に見るより前に皮肉を捕えること

リリースプロセスはフレームワークのコンテンツ面が AI にリアルタイムで語っているものだ。同じ規律で扱う。

### 7. 同じ `@id` に対する重複JSON-LDエンティティを避ける
