...
| 領域 | 主な問い |
|------|---------|
| [1. AI Citation Analysis（AI 引用分析）](#1-ai-citation-analysisai-引用分析) | LLM はどのコンテンツを、どの条件で引用するか |
| [2. Grounding Visibility（根拠付けの可視化）](#2-grounding-visibility根拠付けの可視化) | AI が何を根拠に回答しているかをどう可視化するか |
| [3. LLM Retrieval Optimization（LLM 検索最適化）](#3-llm-retrieval-optimizationllm-検索最適化) | LLM 検索層に対して文書をどう最適化するか |
| [4. AI-native Documentation（AI ネイティブな文書化）](#4-ai-native-documentationai-ネイティブな文書化) | LLM が扱いやすい文書フォーマットとは何か |
| [5. Agent-oriented Information Architecture（エージェント指向の情報アーキテクチャ）](#5-agent-oriented-information-architectureエージェント指向の情報アーキテクチャ) | AI エージェントが操作しやすい情報構造とは何か |

---

## 1. AI Citation Analysis（AI 引用分析）
...
AI 引用観測のデータ収集を開始している。Phase 1 で OSS の `llmo-checker` に Citation Visibility 指標として実装予定。

---

## 2. Grounding Visibility（根拠付けの可視化）
...
## 3. LLM Retrieval Optimization（LLM 検索最適化）
...
## 4. AI-native Documentation（AI ネイティブな文書化）
...
## 5. Agent-oriented Information Architecture（エージェント指向の情報アーキテクチャ）
...
MCP exposure の検索可視性に対する影響を検証中。Phase 1 で Agent Visibility の予備指標を提案予定。
