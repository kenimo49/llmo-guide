...
사용자가 ChatGPT에 당신의 비즈니스에 대해 질문하면, 그 답변은 어디서 오는 걸까? AI 시스템은 세 가지 경로를 통해 콘텐츠를 발견한다. 각 경로는 최적화를 위한 요구 사항이 다르다 — 이 경로들이 바로 [LLMO(Large Language Model Optimization)](/ko/guide/what-is-llmo/)의 "어떻게"에 해당한다.
...
**관련 LLMO 컴포넌트:** 검색 신호, 구조화 포맷, 인용 신호
...
**관련 LLMO 컴포넌트:** 지식 명확성, 구조화 포맷, 검색 신호

## 어떤 경로가 가장 중요한가?

| 경로 | 제어 수준 | 효과 시기 | 주요 LLMO 초점 |
|------|-------------|-----------------|-------------------|
| 학습 데이터 | 낮음 | 수개월~수년 | 지식 명확성 |
...
LLMO는 세 가지 경로를 동시에 최적화한다. LLMO Framework의 [6가지 컴포넌트](/ko/framework/overview/)는 각각 이러한 발견 경로의 특정 측면을 다룬다.
