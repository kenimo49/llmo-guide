...
2025년 10월, Microsoft는 AI 생성 검색 답변(Bing Chat, Copilot)에 콘텐츠가 표시되기를 원하는 콘텐츠 제작자를 위한 공식 가이드라인을 발표했습니다. 이 가이드라인은 LLMO 프레임워크와 밀접하게 일치하며, 여러 LLMO 구성 요소에 대한 벤더 차원의 검증을 제공합니다. (용어가 처음이라면 [LLMO란?](/ko/guide/what-is-llmo/)부터 보세요.)
...
**LLMO와의 정합성:** 이는 Component 2(구조화 포맷)에 직접 대응됩니다. JSON-LD, 시맨틱 HTML, llms.txt 사용에 대한 LLMO 프레임워크의 권고가 Microsoft의 가이드라인에 의해 검증됩니다.
...
**LLMO와의 정합성:** 이는 Component 4(권위 신호)에 대응됩니다. LLMO 프레임워크는 크로스 플랫폼 일관성과 검증 가능한 자격을 핵심 차별화 요소로 강조합니다.
...
**LLMO와의 정합성:** 이는 게시일과 버전 정보를 요구하는 Component 5(인용 신호)와, 정기적으로 업데이트되는 llms.txt 및 사이트맵 파일을 강조하는 Component 3(검색 신호)에 걸쳐 반영됩니다.
...
|------|---------------|---------------|---------|
| 모든 페이지에 JSON-LD 추가 | 구조 | 2. 구조화 포맷 | 높음 |
| 제목 계층 구조 일관되게 사용 | 구조 | 2. 구조화 포맷 | 높음 |
| 자격 정보가 포함된 저자 소개 추가 | 권위 | 4. 권위 신호 | 높음 |
| 게시일 포함 | 최신성 | 5. 인용 신호 | 높음 |
| 산문 형태의 비교를 표로 변환 | 구조 | 2. 구조화 포맷 | 중간 |
| Schema.org Article/Person 마크업 추가 | 구조 + 권위 | 2 + 4 | 중간 |
| 분기별 이상의 콘텐츠 업데이트 | 최신성 | 3. 검색 신호 | 중간 |
| 1차 출처로의 링크 | 권위 | 5. 인용 신호 | 중간 |

## Microsoft 원칙과 LLMO의 대응 관계

```
Microsoft 3대 원칙              LLMO 프레임워크 (6개 구성 요소)
─────────────────────────    ────────────────────────────
Structure (구조)           →   2. 구조화 포맷
                               3. 검색 신호 (일부)
Authority (권위)           →   4. 권위 신호
                               1. 지식 명확성 (일부)
Freshness (최신성)         →   5. 인용 신호
                               3. 검색 신호 (일부)
```

LLMO 프레임워크의 Component 1(지식 명확성)과 Component 3(검색 신호)의 세부 구현은 Microsoft 가이드라인이 다루는 범위를 넘어섭니다. 이는 LLMO가 Bing/Copilot 검색뿐만 아니라 모든 LLM 상호작용의 전체 스펙트럼을 다루기 때문입니다.
...
- [구조화 포맷](/ko/framework/structural-formatting/)
- [권위 신호](/ko/framework/authority-signals/)
