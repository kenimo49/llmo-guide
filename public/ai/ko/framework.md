# LLMO 프레임워크 — 6개 핵심 컴포넌트

## 개요

LLMO 프레임워크는 AI 시스템이 콘텐츠를 발견하도록 만드는 구조적 접근법을 제공합니다. 6개 컴포넌트가 협력해 AI 발견성을 극대화합니다.
...
**목표**: AI가 읽는 모든 면에서 같은 사실이 같은 이야기를 하도록 보장한다.

- AI 전용 면(llms.txt, /ai/*.md)을 HTML과 같은 데이터로부터 생성한다
- 모든 수치·사실 주장에 대해 단일 정규 소스를 유지한다
- 여러 JSON-LD 블록에서 참조되는 엔티티는 하나의 @id를 사용한다
- canonical host와 trailing-slash 정책을 모든 곳에서 강제한다
- 릴리스 전에 cross-file drift를 감지하는 CI gate를 추가한다
